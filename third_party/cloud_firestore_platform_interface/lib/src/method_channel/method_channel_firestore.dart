...
class MethodChannelFirebaseFirestore extends FirebaseFirestorePlatform {
  ...
  Future<T> runTransaction<T>(
    ...
    // Native events may arrive while transactionStoreResult is awaited.
    // Preserve the first outcome, including transport/handler failures.
    void failOnce(Object error, [StackTrace? stack]) {
    ...
    final snapshotStreamSubscription =
      ...
      (event) async {
        ...
        try {
          ...
          try {
            await pigeonChannel.transactionStoreResult(
              transactionId, PigeonTransactionResult.failure, null,
            );
          } catch (_) {
            // Keep the original handler failure if native cleanup also fails.
          }

          // Allow the [runTransaction] method to listen to an error.

          failOnce(error, stack);
        ...
        try {
          await pigeonChannel.transactionStoreResult(
            transactionId, PigeonTransactionResult.success, transaction.commands,
          );
        } catch (error, stack) {
          failOnce(error, stack);
        }
      },
      onError: (Object error, StackTrace stack) => failOnce(error, stack),
    );
